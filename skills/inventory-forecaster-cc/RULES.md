...
| F_DC_LAG        | (2026-05-26) DC inventory lag correction. POS and DC OH data run one week behind. Adjusts W1-W2 by computing adj_DC_OH = DC_OH + Open_PO - LW_POS_Sales, then delta = (target_WOS x rate) - adj_OH, split 50/50 across W1/W2. Amazon: uses Inv_SOH + Inv_OPO - Ordered_Units_LW, target 10 WOS (AMZ_WOS_TARGET_MIN). Retailer: uses OH_Units_LW - POS_Units_LW (no OPO), target 8 WOS (RTL_WOS_TARGET). Fires only when |adj_WOS - raw_WOS| >= 0.5 wks AND |delta/2| >= master_pack. Skips Inactive, OTB(zero), Pre-launch, and Retailer WOS(POS) models. Fired on 958 records in --all run 2026-05-26. |
| F_YOY_CADENCE   | (2026-05-26) LY promo/holiday window timing replication. For items with >=3 non-zero order weeks in LY (hist[:26]), redistributes forecast units WITHIN event windows to match last year's actual week-by-week order shape. Total forecast quantity for each window is preserved; only timing changes. Amazon windows: Prime Day boost weeks + Fall Deal boost weeks (from _get_event_boosts()). Retailer windows: TRADE_FALL_REPLEN_WEEKS (W17-W18) + TRADE_FALL_SEASON2_WEEKS (W21-W22). Single-week windows skipped (no intra-window redistribution possible). Skips Inactive models and records where F58 Tell-AI was applied (planner intent wins). Pack-rounding residual absorbed into largest week. Fired on 659 records in --all run 2026-05-26 across Chewy (174), PetSmart (50), Walmart (50), iHerb (38), Menard (31), Kohl's (23), and more. |
...
- **F81** and above (F77-F80 used 2026-05-24; F_DC_LAG and F_YOY_CADENCE are named rules added 2026-05-26)
- **R10** and above
- **M4+** (M1, M2, M3 only)
- **T5+** (T1, T2, T3, T4 only)
