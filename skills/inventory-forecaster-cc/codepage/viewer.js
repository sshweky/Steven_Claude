...
function renderPage(page) {
  ...
  pageRecs.forEach(r => {
    ...
    tr.innerHTML = `
      <td id="row-badges-${_safeId2}" style="white-space:nowrap;text-align:center;">${r.planner_reply_pending ? '<span class="reply-badge" title="Planner reply awaiting director review">[R]</span>' : ''}${r.manager_reply_pending ? '<span class="mgr-badge" title="Manager flagged - planner action required">[M]</span>' : ''}${r.auto_project ? '<span style="display:inline-block;background:#1b5e20;color:#fff;border-radius:3px;padding:0 4px;font-size:10px;font-weight:700;letter-spacing:0.3px;margin-left:2px;" title="Auto Project: AI projections will replace manual projections on each forecast run">[A]</span>' : ''}</td>
