...
function adaptRow(row) {
  ...
  return {
    ...
    baseline_override:      F.BASELINE_OVERRIDE      ? num(row, F.BASELINE_OVERRIDE)      : 0,
    baseline_override_date: F.BASELINE_OVERRIDE_DATE ? str(row, F.BASELINE_OVERRIDE_DATE) : '',
...
async function zeroDuplicateManPrj(key, safeKey) {
  ...
  // Stage zeros for all conflict weeks
  ...
  // Show saving state
  const btn = document.getElementById('zero-dup-btn-' + sk);
  if (btn) {
  ...
  // If no dirty edits remain for this key the save succeeded
  const stillDirty = [...DIRTY_EDITS.values()].some(e => e.key === key);
  if (!stillDirty) {
    // Clear conflict data and remove the entire warning banner
    r.po_prj_conflicts = [];
    ...
    // Save failed -- let the user try Save All manually
    if (btn) {
      btn.textContent = 'Save failed -- try Save All';
...
function showNewPrjModal() {
  ...
  document.getElementById('npm-acct-input').value        = '';
  document.getElementById('npm-acct-input').disabled     = true;
  document.getElementById('npm-acct-input').placeholder  = '-- select division first --';
