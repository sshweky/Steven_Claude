...
function renderDetail(r) {
  ...
  var demandHtml='<table class="subtbl"><tr><th>Demand</th><th class="right">Qty</th><th>Shipments</th><th class="right">Qty</th><th>Date</th><th>Value</th></tr><tr><td>Prj / Wk</td><td class="right"><b>'+fmt(r.prj_wk)+'</b></td><td>Shpd / Wk L4</td><td class="right"><b>'+fmt(r.shp_wk_l4)+'</b></td><td>Last Shp Date</td><td>'+fmtDate(r.last_shp_date)+'</td></tr><tr><td>Max Prj / Wk</td><td class="right">'+fmt(r.max_prj_wk)+'</td><td>Shpd / Wk L13</td><td class="right"><b>'+fmt(r.shp_wk_l13)+'</b></td><td>1st Shpd Date</td><td>'+fmtDate(r.first_shpd_date)+'</td></tr><tr><td>+/- Prj L4w</td><td class="right">'+fmt(r.prj_l4w_change)+'%</td><td>Total Shpd L4</td><td class="right">'+fmtInt(r.tot_shpd_l4)+'</td><td>Date 1st Rcvd</td><td>'+fmtDate(r.date_1st_rcvd)+'</td></tr><tr><td>Prj 26 Wks</td><td class="right">'+fmt(r.prj_26wks)+'</td><td>Total Shpd L13w</td><td class="right">'+fmtInt(r.tot_shpd_l13w)+'</td><td>Last Whs Rcvd</td><td>'+fmtDate(r.last_whs_rcvd)+'</td></tr><tr><td>Manual demand (rollup)</td><td class="right">'+fmt(r.manual_demand_26w)+'</td><td>Total Shpd LTD</td><td class="right">'+fmtInt(r.tot_shpd_ltd)+'</td><td>1st Out Date</td><td>'+fmtDate(r.first_out_date)+'</td></tr><tr><td>Demand (Inv Flow 26w Sum)</td><td class="right">'+fmt(r.demand_26w)+'</td><td colspan="2"></td><td>Last OOS Date</td><td>'+fmtDate(r.last_oos_date)+'</td></tr></table><div class="stat-text" style="margin-top:4px;"><b>Days OOS till Next Rcpt:</b> '+fmt(r.days_oos_next_rcpt)+' / <b>Days OOS L12m:</b> '+fmt(r.days_oos_l12m)+'</div>';
