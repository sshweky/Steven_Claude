...
function renderDetail(r) {
  ...
  var stockStatusBox='<div style="flex:1;min-width:200px;background:#f3e5f5;border:1px solid #e4e7eb;border-radius:4px;padding:8px 10px;"><div style="font-size:10px;font-weight:700;color:#9e9e9e;text-transform:uppercase;letter-spacing:0.6px;margin-bottom:6px;">Stock Status</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:0 10px;">'+
  ...
  var atsHtml='<table class="subtbl"><tr><th>Position</th><th class="right">Qty</th><th>Position</th><th class="right">Qty</th><th>WOS Metric</th><th class="right">Value</th></tr><tr><td>Qty OH (total)</td><td class="right"><b>'+fmtInt(r.qty_oh)+'</b></td><td>I/T (in transit)</td><td class="right">'+fmtInt(r.it_qty)+'</td><td>ATS WOS OH</td><td class="right">'+fmt(r.ats_wos_oh)+'</td></tr><tr><td>ATS Qty OH</td><td class="right">'+fmtInt(r.ats_qty_oh)+'</td><td>I/W (in work)</td><td class="right">'+fmtInt(r.iw_qty)+'</td><td>ATS WOS OH+OO</td><td class="right">'+fmt(r.ats_wos_oh_oo)+'</td></tr><tr><td>ATS Now</td><td class="right"><b>'+fmtInt(r.ats_now)+'</b></td><td>I/T + I/W</td><td class="right">'+fmt(r.it_iw)+'</td><td>ATS WOS OH+OO (w/ kits)</td><td class="right">'+fmt(r.ats_wos_oh_oo_w_kits)+'</td></tr><tr><td>ATS OH + OO</td><td class="right">'+fmtInt(r.ats_oh_oo)+'</td><td>I/W + I/T w/ Kits</td><td class="right">'+fmt(r.it_iw_kits)+'</td><td>ATS WOS (w/o test/excl)</td><td class="right">'+fmt(r.ats_wos_oh_oo_wo_test)+'</td></tr><tr><td>ATS OH + OO (w/ kits)</td><td class="right">'+fmtInt(r.ats_oh_oo_w_kits)+'</td><td>Open Cust PO Qty</td><td class="right">'+fmtInt(r.open_cust_po_qty)+'</td><td>ATS OH + I/T Booked WOS</td><td class="right">'+fmt(r.ats_oh_it_booked_wos)+'</td></tr><tr><td>ATS Qty (not alloc\'d)</td><td class="right">'+fmtInt(r.ats_qty_not_alloc)+'</td><td>Hold Order Qty</td><td class="right '+(r.hold_qty>0?'pri-MEDIUM':'')+'">'+fmtInt(r.hold_qty)+'</td><td>Opt WOS</td><td class="right"><b>'+fmt(r.opt_wos)+'</b></td></tr><tr><td>NJ ATS OH</td><td class="right">'+fmtInt(r.nj_ats_oh)+'</td><td>Test Order Qty</td><td class="right">'+fmt(r.test_order_qty)+'</td><td>Opt OH</td><td class="right">'+fmt(r.opt_oh)+'</td></tr><tr><td>CA ATS OH</td><td class="right">'+fmtInt(r.ca_ats_oh)+'</td><td>Exclude PO from WOS</td><td class="right">'+fmt(r.exclude_po_wos)+'</td><td>LT (Wks) / CNY / LT+Opt</td><td class="right">'+fmt(r.lt_wks)+' / '+fmt(r.cny_weeks)+' / '+fmt(r.lt_opt_weeks)+'</td></tr></table>';
