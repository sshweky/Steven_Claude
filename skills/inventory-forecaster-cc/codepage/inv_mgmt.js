...
function renderDetail(r) {
  ...
  var itemDataBox=kvBox('Item Data','<div style="display:grid;grid-template-columns:1fr 1fr;gap:0 14px;"><div>'+kvRow('Inner Pack',r.inner_pack||'&#8212;')+kvRow('Master Pack',r.master_pack||'&#8212;')+kvRow('MOQ',r.moq?fmt(r.moq):'-')+kvRow('Opt OH',fmt(r.opt_oh))+'</div><div>'+kvRow('Opt WOS',fmt(r.opt_wos))+kvRow('LT (Wks)',fmt(r.lt_wks))+kvRow('LT + Opt Wks',fmt(r.lt_opt_weeks))+(r.upc?kvRow('UPC #',esc(r.upc)):'')+( r.gtin?kvRow('GTIN #',esc(r.gtin)):'')+'</div></div>');
