...
var COLS = [
  ...
  { id:'description', label:'Description', align:'left',
    ...
    render:function(r){return '<td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="'+esc(r.description)+'">'+esc(r.description)+'</td>';} },
  { id:'brand', label:'Brand', align:'left', get:function(r){return r.brand;}, render:function(r){return '<td>'+esc(r.brand)+'</td>';} },
  { id:'country', label:'Country', align:'left', get:function(r){return r.country;}, render:function(r){return '<td>'+esc(r.country)+'</td>';} },
  { id:'inv_manager', label:'Inv Mgr', align:'left', get:function(r){return r.inv_manager;}, render:function(r){return '<td>'+esc(r.inv_manager)+'</td>';} },
  { id:'status_sub', label:'Status / Sub', align:'left',
    get:function(r){return r.item_status_flow||r.item_status||'';},
    filterValue:function(r){return (r.item_status_flow||r.item_status||'')+' '+(r.sub_status||'');},
    render:function(r){return '<td>'+esc(r.item_status_flow||r.item_status||'')+(r.sub_status?'<br><span style="color:#888;font-size:10px;">'+esc(r.sub_status)+'</span>':'')+'</td>';} },
  ...
  { id:'prj_l4w_change', label:'+/- L4w', align:'right', numeric:true, get:function(r){return r.prj_l4w_change;},
    render:function(r){var l4w=r.prj_l4w_change;var arr=l4w>5?'<span style="color:#2e7d32;font-weight:700;">&#9650;</span>':l4w<-5?'<span style="color:#c62828;font-weight:700;">&#9660;</span>':'';return '<td class="right">'+arr+' '+fmt(l4w)+'%</td>';} },
  ...
  { id:'pipeline_excess', label:'OH+OO Excess', align:'right', numeric:true,
    ...
    tooltip:'OH+OO Excess = (Week 1 OH + all open PO qty) - 26w projected demand - safety stock\nPositive = overstock. > 2,500 triggers Overstock flag.',
    render:function(r){return '<td class="right '+(r.pipeline_excess>2500?'pri-HIGH':'')+'">'+fmt(r.pipeline_excess)+'</td>';} },
...
function buildTableHead() {
  ...
  cols.forEach(function(c){
    ...
    var tip=c.tooltip?' title="'+c.tooltip.replace(/"/g,'&quot;')+'" style="cursor:help;"':'';
...
function renderDetail(r) {
  ...
  return '<div class="dwrap"><div class="section" style="padding:10px 14px;"><div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-start;">'+identityBox+itemDataBox+overstockBox+'</div></div><div class="section"><h3>&#128230; Inventory Flow <span style="font-size:10px;font-weight:400;color:#888;">— hover Expected Receipts cells for PO detail · hover Prj Demand cells for customer breakdown</span></h3><div style="overflow-x:auto">'+invFlow+'</div>'+kpiStrip+'</div><div class="section"><h3>&#128202; Inventory Positions &amp; WOS</h3>'+atsHtml+'</div><div class="section"><h3>&#128200; Demand &amp; Shipments</h3>'+demandHtml+'</div><div class="section"><h3>&#128197; Aged Inventory</h3>'+agedInvHtml+'</div><div class="section"><h3>&#128680; OOS Gap Detail</h3>'+gapDetail+'</div><div class="section"><h3>&#127919; Recommended Actions</h3>'+recs+'</div></div>';
}
