...
# After that, the forecaster runs automatically every 2 hours Mon-Fri, 6 AM - 8 PM.
#
# Usage:
...
# -- Register (prompts for Windows password once) -----------------------------
...
$Password = Read-Host "Enter your Windows login password (required for 'run when locked')" -AsSecureString
...
$PlainPw = $null  # clear from memory
...
Write-Host "  # wait a minute, then check logs\ for the output file"
