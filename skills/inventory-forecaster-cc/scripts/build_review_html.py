...
.btn { padding: 8px 16px; border: 0; border-radius: 6px; font-size: 13px; font-weight: 600; cursor: pointer; transition: filter .15s, transform .05s, background .15s, border-color .15s; }
.btn:hover { filter: brightness(1.08); }
.btn:active { transform: translateY(1px); }
.btn.approve { background: #2e7d32; color: white; padding: 10px 24px; font-size: 14px; min-width: 110px; }
.btn.reject  { background: #c62828; color: white; padding: 10px 24px; font-size: 14px; min-width: 110px; }
/* Modify is secondary: smaller, outline-style, gray text */
.btn.modify  { background: transparent; color: #6b7280; padding: 6px 12px; font-size: 12px; font-weight: 500; border: 1px solid #cbd5e1; }
.btn.modify:hover { background: #f3f4f6; color: #374151; border-color: #94a3b8; filter: none; }
/* When modify box is open and we are inside it, keep the Preview Update button bold orange */
.modify-box .btn.modify { background: #ed6c02; color: white; padding: 8px 16px; font-size: 13px; font-weight: 600; border: 0; }
.modify-box .btn.modify:hover { background: #ed6c02; color: white; filter: brightness(1.08); border: 0; }
...
"""
    ...
    body = (
        ...
        '    <button class="btn save" id="save-btn" disabled>Lock in Decisions</button>'
        '    <div class="help-text" id="commit-help">All proposals must be decided (Approve/Reject/Modify) before locking in.</div>'
        '    <div id="commit-banner" style="display:none;margin-top:14px;padding:12px 16px;background:#e8f5e9;border:1px solid #2e7d32;border-radius:6px;color:#1b5e20;font-size:14px;">'
        '      <b>Decisions locked in.</b> Now type <code style="background:#fff;padding:1px 6px;border-radius:3px">commit decisions</code> in Claude Code. I will pull the payload from this tab via the browser bridge and apply the changes.'
        '      <details style="margin-top:8px"><summary style="cursor:pointer;font-weight:600">Show raw payload (fallback if browser bridge fails)</summary><pre id="commit-payload" style="margin-top:8px;padding:10px;background:#f9fafb;border-radius:4px;font-size:11px;overflow:auto;max-height:240px;white-space:pre-wrap"></pre><button class="btn" style="background:#0d47a1;color:white;margin-top:6px" onclick="copyPayload()">Copy payload to clipboard</button></details>'
        '    </div>'
        '  </div>'
        '</div>'
    )
...
const STATE = { decisions: {}, modifications: {}, params: {}, charts: {}, originalRuleText: {} };
PROPOSALS.forEach(p => {
...
const STATUS_MAP = { approve: 'approved', reject: 'rejected', modify: 'modified' };
function decide(id, d) {
  STATE.decisions[id] = d;
  if (d !== 'modify') STATE.modifications[id] = null;
  const past = STATUS_MAP[d] || d;
  const card = document.getElementById('card-' + id);
  card.classList.remove('approved','rejected','modified');
  card.classList.add(past);
  const status = document.getElementById('status-' + id);
  status.className = 'tag status-' + past;
  status.innerText = past.toUpperCase();
  updateGlobal();
}
...
// PREVIEW UPDATE flow: POST modification text directly to localhost preview server.
// Server interprets the text, recomputes, and returns updated values. Card refreshes
// without needing a Claude turn. Card stays PENDING; user clicks Approve / Reject.
const PREVIEW_SERVER_URL = 'http://localhost:8744/preview';
...
async function previewUpdate(id) {
  ...
  status.innerHTML = '<span style="color:#0d47a1;font-weight:600">Computing update...</span>';

  // POST to the local preview server
  try {
    const resp = await fetch(PREVIEW_SERVER_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        id: id, key: p.key, rule_id: p.rule_id,
        rule_fn_id: p.rule_fn_id, scope_key: p.scope_key,
        text: text,
      }),
    });
    ...
    if (!r.ok) {
      const msg = r.errors ? r.errors.join('; ') : (r.error || 'unknown error');
      status.innerHTML = '<span style="color:#c62828;font-weight:600">Could not interpret:</span> ' + msg + (r.hint ? ' <i>' + r.hint + '</i>' : '');
      return;
    }
    // Apply the result to PROPOSALS + refresh the card
    ...
    if (typeof r.sys_scope === 'number') p.sys_scope = r.sys_scope;
