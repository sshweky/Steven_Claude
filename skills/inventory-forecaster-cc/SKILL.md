...
- **Phase 2.6d (Inventory Flow per-mstyle for F37 v2b direct-BegInv)** -- QB REST on `QB_INV_FLOW_TABLE` = `bpsaju5pm` (InventoryTrack.Inventory_Flow). FIDs: Mstyle=20, Wk1..Wk26 (134,8,9,10,110-119,120-131,119 -- QB-formula Beg Inv per week, read directly), RcvWk0-26 (295/28/35/36/50/51/65..85), Opn_Wk0-26 (296/30/37/39/38/87/89..109), LT_Trans_Days=225. `fetch_inv_flow_qb_rest()` returns `beg_inv_wks[26]` (direct QB values), `rcv[26]`, `opn[26]`, `lt_trans_days`. RcvWk0/Opn_Wk0 (prior-week residuals) roll into the W1 slot per planner convention.
...
|---|---|---|
| 2026-05-27 | F89, F90 | **F89 -- Croston MAN-PRJ directional gate (AI Training Rec #1, 2026-05-27):** Fires after Croston's runs when (1) a spike order exists in L13W history (any single week >= L13W mean * 3x) AND (2) AI 26w total is above MAN PRJ 26w total. Caps AI at MAN PRJ * 1.05. Gate prevents the cap from firing on records where AI is already at/below MAN. Validated against 295 active Croston records: 80 flagged, narrows MAN-AI variance from -106.5% (-178,417u) to -4.9% (-8,220u). Fired 80 records on 2026-05-27 --all run. **F90 -- Heuristic trend-aware baseline (AI Training Rec #2, 2026-05-27):** Fires after Heuristic runs when AI 26w total exceeds MAN PRJ 26w total by > 15%. Switches the forecast from the flat L13W baseline to max(L4W_ord, L13W_ord) * 26, applying the existing ordering-pattern shape. Only applies when the new baseline moves AI closer to MAN (abs-gap check). Validated against 98 active Heuristic records: 86 flagged, narrows MAN-AI variance from -55.0% (-186,923u) to -36.8% (-124,936u). Fired 5 records on 2026-05-27 --all run (many of the 86 training-estimate records were already corrected by upstream rules). |
| 2026-05-26 | F_POG_END_ZERO, F_POG_END_WARN | **F_POG_END_ZERO / F_POG_END_WARN -- POG End Date behavior (2026-05-26):** Two paired rules that fire based on `POG_End_Date` and `Status_Cust`. **F_POG_END_ZERO** fires when `Status_Cust` starts with "FD" (Future Delete) AND `POG_End_Date` is within the 26-week horizon. Hard-zeros AI PRJ from W(cutoff) through W26, where cutoff = 6 weeks before POG End Date. Emits a driver message showing how many units were removed. Result dict: `pog_end_zero_wk` (1-based cutoff week), `pog_end_exposure` (units removed), `pog_end_warn=False`. **F_POG_END_WARN** fires when `Status_Cust` starts with "A" (Active, not FD) AND `POG_End_Date` is within 26-week horizon AND AI PRJ has non-zero units at/past the cutoff. No forecast change -- fires a giant driver warning about overstock exposure (`pog_end_exposure` units). Result dict: `pog_end_warn=True`, `pog_end_zero_wk`, `pog_end_exposure`. Codepage visibility: Forecast Manager (viewer.js, page 49) shows a dark-red banner (WARN) or purple banner (ZERO applied) inside the POG block. Inventory Management (inv_mgmt.js, page 56) adds `POGEndDate` FID 1595 to the PRJ query, detects per-customer warn signals at data-load time, and shows a red section-level warning in `renderDetail` for any mstyle with at least one customer approaching their POG cutoff. Both pages deployed 2026-05-26. |
| 2026-05-26 | F_POG_FUTURE | **F_POG_FUTURE -- future POG order schedule (2026-05-26):** Fires for non-Amazon, non-Inactive, non-OTB retailers when `POG_Launch_Date` is a future date within the 26-week horizon. Overrides the base model with a 3-phase event-driven schedule: (1) **ISO fill** ~4 weeks before POG start -- qty = `Estimated_ISO` if set, else `Store_Count x Master_Pack`, snapped to mp. Skipped if ISO already placed (detected via: max order in L8W history >= 50% of ISO qty, OR open PO within +-1 week of ship window). (2) **Dead zone** -- 3 weeks after ISO ships, forecast = 0. (3) **Ramp** starting at POG start week -- base = `Store_Count x UPSPW x 75%` snapped to mp, with category seasonal profile + T5/event lifts layered on top. Falls back to L13W SHP average if UPSPW = 0. **POS adjustment** (once data arrives): if `POS_Units_LW > ramp_base` adjust up immediately; if `POS_Units_LW < ramp_base` hold for 4 weeks from ISO ship date then adjust down. Phase 1 now fetches `Estimated_ISO` (FID auto from field map) and `UPSPW` (FID auto) alongside `Store_Count`. |
| 2026-05-26 | F_PO_ZERO_ALL | **F_PO_ZERO_ALL -- final PO-zero safety net, all weeks (2026-05-26):** Replaces the former W1-only `F_W1_PO_ZERO` guard. Runs as the LAST rule in the pipeline. Loops W1-W26: any week where `_opn_row_wk[w] > 0` and `fcst[w] > 0` is zeroed unconditionally. VP-Q4 does an earlier zeroing pass, but downstream rules (F69-WOS rescale, F37, F_YOY_CADENCE, etc.) can re-inflate PO weeks -- this is the unconditional backstop. Fires for all customers. On fire, merges zeroed weeks into `meta["po_zeroed_weeks"]` and updates `po_total_removed`/`po_total_qty` so writeback and alert display stay consistent. |
...
| 2026-05-26 | F37 v2b (direct-BegInv), Phase 2.6d Inv Flow REST | **F37 v2a (fresh-cascade, 2026-05-26 morning) SUPERSEDED by v2b (same day):** v2a replaced stale `Inv_Wk*` reads with an in-Python cascade from Wk1 using the current AI projection as surrogate demand -- compounded errors week-over-week. v2b reads QB-formula Beg Inv DIRECTLY per week from Wk1..Wk26 FIDs in Inventory Flow: capacity = `max(0, BegInv_W[w] + rcv[w] - opn[w])`. No cascade, no AI-demand surrogate. Phase 2.6d fetches all 26 Beg Inv FIDs (134,8,9,10,110-119,120-131,119) + RcvWk0-26 + Opn_Wk0-26 + LT_Trans_Days=225 via QB REST; returns `beg_inv_wks[26]`, `rcv[26]`, `opn[26]`. |
